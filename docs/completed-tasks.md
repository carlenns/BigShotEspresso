...
## Taste-selector vocabulary model — decision record — 2026-08-27

Docs only, no code. New file `docs/architecture/taste-selector-vocabulary-model.md` recording Carl's decision on standardizing taste vocabulary for future global aggregation and fixing the destructive-delete behaviour flagged in the Part 2 audit (`shot_taste_selectors.taste_selector_id` `ON DELETE CASCADE` silently strips a tag from all historical shots).

Decision captured: (1) archive instead of destructive delete — additive nullable column, **recommended `archived_at timestamptz NULL`** over `is_active boolean` because it records *when* a selector was retired (the audit-trail reason we're not hard-deleting) and needs no backfill; boolean is an acceptable substitute that only loses the timestamp. (2) master list is canonical + versioned — needs a stable deployment-independent `canonical_key` (current `id` is a local `serial`) and a version stamp; mechanism is future work. (3) custom selectors are account-scoped, flagged personal, excluded from global upload; `sensory_notes` is the escape hatch; depends on ADR-0009 accounts. (4) add an `origin` column (`standard` | `custom`) **now** in the same migration, backfilled from `is_default`, because `is_default` currently overloads "canonical marker" + "UI delete gate".

Doc sections: Problem / Decision (D1–D4) / Archive column recommendation / Dependency / Immediate implementable slice (no accounts: archive column + `origin` column + archive/restore API + Taste Selectors page UI + tag-picker filter + gated hard-delete + tests) / Out of scope (canon versioning, custom-creation-as-account-feature, personal→canon promotion, global upload pipeline, dropping `is_default`) / Cross-reference. States explicitly it does not authorize implementation or the migration.
