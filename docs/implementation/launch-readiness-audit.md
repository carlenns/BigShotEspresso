...
| **Sour ⇏ Reference/Signature**; Sour still analytically valid if Status/Fault Good | **CLOSED 2026-08-27 (commit pending)** — enforced in `normalizeShotInput` (`routes/shots.ts`) | *Historical (as found):* `normalizeShotInput` had **no `sourShot` handling** — a direct `POST/PATCH /shots` with `{sourShot:true, isReference:true, signatureShot:true}` was stored as-is; enforcement existed only in `ShotForm.tsx`. *Now:* `normalizeShotInput` forces `isReference`/`signatureShot` false when `sourShot === true` (both POST and PATCH); CSV import path unaffected. The "still valid" half was always correct: `computeIncludeInAnalysis` ignores `sourShot`, so a Sour shot with Status/Fault Good is still included — not over-excluded. |
...
| **Hopper phase labels** limited to Phase 1/2/3, End of Bag, Single Bag Phase, Custom | **CLOSED 2026-08-27 (commit pending)** — enforced via `invalidHopperPhase` in `routes/hopper.ts` | *Historical (as found):* `Bags.tsx` `HOPPER_PHASE_OPTIONS` (L18) constrained only the UI dropdowns; the API contract is `phase: zod.string().nullish()` (`lib/api-zod/src/generated/api.ts` L1056/1068/1082/1095) and `routes/hopper.ts` POST (L47) / PATCH (L103) applied **no allow-list**. *Now:* `routes/hopper.ts` rejects a `phase` outside the approved set via `invalidHopperPhase` on the interactive POST/PATCH paths; the CSV import path is unaffected, preserving `docs/table-relationships.md` L70 ("values observed or approved") import tolerance. |

### Assessment

- Rules 1, 2, 4: enforced/consistent where it matters (server for 1–2; analytics for 4). No action needed for owner-alpha.
- Rules 3 (Sour exclusion) and 5 (phase labels): **CLOSED 2026-08-27 (commit pending)** — enforced server-side in `normalizeShotInput` (`routes/shots.ts`) and `invalidHopperPhase` (`routes/hopper.ts`) respectively, both on the interactive POST/PATCH paths only; the CSV import path is deliberately unaffected so historical/imported values still survive. As found, both were client-only — consistent with the project's "curate in the UI, keep the API permissive" posture, and not a launch blocker for owner-only use, but worth closing before Tier 2 (outside testers hitting the API directly), which is what landed.
...
Review of whether pre-filling `grindSetting` / `grindTime` / `initialGrindWeight` / `topUpGrind` / `timeAdj` / `temperature` / `dose` / `yield` onto every new shot distorts any Dashboard analytic. Reasoned from `artifacts/coffee-log/src/pages/ShotForm.tsx` (current, `a0fc7ba`) and `artifacts/api-server/src/routes/dashboard.ts`. Findings only — no code proposed.

**Decision (2026-08-27):** Carl chose full WYSIWYG pre-fill of every defaulted field, `yield` included, with a top-of-form explanatory note. The `yield`-window / confidence self-reinforcement flagged below (rated shots that keep the default collapsing `bestYieldRange` / `timingWindows.yieldRange` to a point and inflating `confidence`) is an **accepted logging-discipline tradeoff, not a blocker**. Possible future mitigation — discount exact-repeat values when computing range confidence — is noted but **out of scope**.

### How each field actually reaches a saved shot

- **Tier A — hard form default, submitted on every new shot whether or not the user looks at it:** `dose` (18 → bag `defaultDose` → `settings.defaultDose`), `yield` (36 → latest analysis-eligible shot's yield via `/dashboard/intelligence` → bag `defaultYield` → `settings.defaultTargetYield`), `temperature` (94 → bag `defaultTemp` → `settings.defaultBrewTemp`). These are in `useForm` `defaultValues` and/or overwritten by the bag auto-fill effect; the field is never empty, so it is always in the POST body.
- **Tier B — written by the bag auto-fill effect when a bag is selected (a sole active bag now auto-selects):** `grindSetting` ← `bag.currentGrindSetting`, `grindTime` ← `bag.currentGrindTime`. `bag.currentGrindSetting` is itself carried forward from the previous shot by `carryForwardActiveBagGrindDefaults` in `routes/shots.ts`.
- **Tier C — only committed if the user focuses / taps / steps the field (empty and omitted from the POST otherwise):** `initialGrindWeight`, `topUpGrind`, `timeAdj` (seeded via `seedSuggestedNumber` on `onPointerDown` / `onFocus`, or via a `NumberStepper` +/- press).
- **Measured pour fields — suggestion-only:** `pourDelay` / `pourTime` / `flowTime` show the last shot's value as a `placeholder` and `NumberStepper suggestedValue`, but the field value itself starts empty; nothing is submitted unless the user interacts.

### Analytic-by-analytic

1. **Grind journey / drift (`grindDrift`, `bagComparison`, grind-change context).** Pool: analysis-eligible shots for the bag with `grindSetting != null`. `drift = currentSetting − startSetting`; `changesCount` / `largestMove` count only consecutive-shot diffs ≥ 0.005. Because `grindSetting` is carried forward, a run of untouched shots records the *same* value, so drift correctly reads ~0 and `changesCount` does not inflate. **Not distorted** — the pre-fill makes drift accurate when the grinder genuinely wasn't moved. Residual risk is data-entry, not analytics: if the user *did* nudge the grinder but didn't update the field, the shot silently records a stale setting and drift under-reports. Pre-fill converts that from a visible blank into an invisible wrong value.
2. **Best-yield range & yield timing window (`bestYieldRange`, `timingWindows.yieldRange`).** Pool: rated-8+ shots (with same-bean / all-reference fallback), `Number(s.yield) > 0`, then `robustRange` (IQR filter, p25–p75 "peak cluster", `confidence` = High at ≥10 values). **This is the one real concern.** `yield` is Tier A, so every rated-8+ shot carries a yield value; if the user pulls good shots but never adjusts the field, they all record the *identical* default (e.g. 36.0). `robustRange` then returns `min = max = 36.0` with `confidence: "High"` after 10 shots. The **center is not wrong**, but the **spread is fabricated (zero) and the confidence is fabricated (High from repetition)**. The Dashboard shows "Best yield 36.0–36.0 g · High confidence", which reads as a hard-won measured window but is just the default echoed back. `dose` has the same shape but is less prominent in range analytics.
3. **Dose-correction (`overGrindRemoved`, `doseCorrectionType`, `beanMassConsumed`).** `calculateDoseCorrection` returns `{}` unless the user entered a real `initialGrindWeight` (Tier C), so **no dose-correction fields are auto-generated** and those analytics are **not distorted**. Two small notes: (a) `beanMassConsumed` (Bag Progress consumed / remaining / completion %) sums `dose` for *every* logged shot, so it silently under/over-counts by the gap between the default dose and the real dose when the user doesn't edit; Bag Progress is already documented as reconciliation-only, so stakes are low. (b) If a user taps `initialGrindWeight` and it seeds to the same number as `dose`, the shot records `doseCorrectionType: "None"` — harmless today (nothing counts it) but noise for any future "how often do I correct dose" metric.
4. **Timing windows — pour delay / pour time / flow time (`timingWindows.*Range`, `bestPourDelayRange`).** These read `pourDelay` / `pourTime` / `flowTime`, which stay **suggestion-only**. Untouched → `null` → removed by the `.filter((v) => v > 0)` before `robustRange`. So these windows contain only genuinely-entered measurements. **Confirmed: keeping the pour fields suggestion-only is the correct call.** If they were pre-filled the way `yield` is, they would fabricate timing spread and confidence in exactly the same way finding 2 describes. This is the right side of the line: *measured outputs* should not be pre-filled into the stored row; *setup inputs* (target dose, temp, grind setting) pre-filling is defensible.
5. **`bestShot` recipe card / `shotComparison.latestShot`.** Not aggregates, so no range distortion, but: the "repeat this shot" recipe and the "latest vs reference" card will surface the default `dose` / `yield` / `ratio` (e.g. 2.00) when the underlying shot was logged without adjusting them. Cosmetic-to-mild.

### Needs Carl's attention

- **`yield` (and to a lesser extent `dose`) being a hard pre-filled default rather than suggestion-only is a genuine analytics-honesty issue.** Rated shots logged without adjusting yield collapse `bestYieldRange` / `timingWindows.yieldRange` to a single point and push `confidence` to High purely by repeating the default. This conflicts with the project's stated principle that the app must not present more certainty than the data supports. Decision for Carl: should measured outputs (`yield`) be suggestion-only like the pour timings, accepting that some shots then store no yield — or is the current behaviour acceptable because a disciplined owner always adjusts it? (This review does not propose an implementation.)
- **Lower priority:** `beanMassConsumed` inherits the same "assume default dose" gap; and pre-fill in general converts "field left blank" into "field silently holds a stale value", which removes the app's ability to tell *unchanged* from *unrecorded* for `grindSetting` / `dose` / `yield`.
